                ...
                if scan.endswith(".jpg"):
                    ...
                    for j in range (0, img_height):
                        for i in range(0, img_width):
                            ...
                            if img_pixel[i, j][0] <= 50:
                                if img_pixel[i, j][1] <= 50:
                                    if img_pixel[i, j][2] <= 50:
                                        black_pixel_count = black_pixel_count +1
                    
                    # Calculates porosity   
                    img_pixel_total = img_height * img_width
                    porosity =((black_pixel_count / (img_pixel_total - red_pixel_count))) * 100
                ...
                if scan.endswith(".jpg"):
                    ...
                    for j in range (0, img_height):
                        for i in range(0, img_width):
                            ...
                            if img_pixel[i, j] == (254, 0, 0):
                                red_pixel_count = red_pixel_count + 1
                            if img_pixel[i, j][0] <= 60:
                                if img_pixel[i, j][1] <= 60:
                                    if img_pixel[i, j][2] <= 60:
                                        black_pixel_count = black_pixel_count +1
...
def sample_selection():
    ...
    for element in os.listdir(path):
        ...
        for ebene in os.listdir(path + element):
            ...
            if "Ebene" in ebene:
                ...
                for scan in os.listdir(path + element + "/" + ebene):
                    
                    if scan.endswith(".jpg"):
                        
                        # Opens the current scan
                        full_path = path + element + "/" + ebene + "/" + scan
                        ...
                        # Gets the number of scans
                        scan_count = len([scan for scan in os.listdir(path + element + "/" + ebene)])
                        ...
                        wb.save("porosity_data_sample_" + str((path + element).split("/")[6]).split(" ")[0] + "_" + str((path + element).split("/")[6]).split(" ")[1] + ".xlsx")

                        print("Progress: " + str(counter) + " / " + str(scan_count) + ", " + str(round(((counter / scan_count) * 100), 1)) + " %")



selection = input("Surface(1) / Element(2) / Sample(3) selection: ")
if selection == "1":
    surface_selection()
elif selection == "2":
    element_selection()
